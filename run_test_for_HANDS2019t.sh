...
# multi thread
mkdir -p cache1/part
split -l 5000 ${result_path0} cache1/part/result.txt.part -d -a 2
for ((i = 0; i < 25; i += 1)); do
  index=$(echo $i | awk '{printf("%02d",$1)}')
  python hand_region_augment.py \
    -v \
    --dst_dir cache1/hands19task1/test_images_augment \
    --joint_list_path cache1/part/result.txt.part${index} &
done
wait
# rm -rf cache1/part
